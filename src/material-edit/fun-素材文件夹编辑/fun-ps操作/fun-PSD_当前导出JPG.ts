...
export function FUN_当前PSD导出JPG(props: { materialPath: string }) {
	...
	const psdStem = path.basename(props.materialPath, path.extname(props.materialPath));
	const targetImagePath = path.join(psdDir, `${psdStem}.jpg`);

	const exportOptions = new winax.Object("Photoshop.ExportOptionsSaveForWeb.200");
