...
export const XQ_制作效果图 = createServerOnlyFn(async (props: InferRouterInputs<typeof ORPC_制作详情>) => {
	...
	const remainder = props.rows >= 2 ? props.effectImageList.length % props.rows : 0;
	...
	while (i < props.effectImageList.length) {
		...
		const currentItemWidth = i === 0 && remainder > 0 ? firstRowItemWidth : itemWidth;
		i += take;

		const processedRow = await Promise.all(
			row.map(async (img) => {
				console.log(`制作详情：${img.imagePath}`);
				const ratio = img.imageRatio || (img.height > 0 ? img.width / img.height : 1);
				const targetHeight = Math.round(currentItemWidth / ratio);

				let pipeline = sharp(img.imagePath).resize(currentItemWidth, targetHeight);

				if (props.borderRadius > 0) {
					const mask = Buffer.from(`<svg><rect x="0" y="0" width="${currentItemWidth}" height="${targetHeight}" rx="${props.borderRadius}" ry="${props.borderRadius}" /></svg>`);
	...
	for (const row of rowGroups) {
		...
		row.forEach((img, index) => {
			const x = props.outerSpacing + index * (img.itemWidthUsed + props.innerSpacing);
			// 底部对齐：当前行最大高度 - 图片自身高度
