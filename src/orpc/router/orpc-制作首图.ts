...
import { ST_1大1小 } from "#/st-制作首图/st-制作布局/st-1大1小";
import { ST_1大列2小列 } from "#/st-制作首图/st-制作布局/st-1大列-2小列";
import { ST_列自适应 } from "#/st-制作首图/st-制作布局/st-列自适应";
import { ST_固定裁剪 } from "#/st-制作首图/st-制作布局/st-固定裁剪";
import { ST_横版1221 } from "#/st-制作首图/st-制作布局/st-横版1221";
import { RUN_制作黑鲸首图 } from "#/st-制作首图/st-黑鲸/run-制作黑鲸首图";
...
export const ORPC_制作首图 = os
	...
	.handler(async (ctx) => {
		const layoutMap: Record<string, () => Promise<Buffer>> = {
			固定裁剪: async () => await ST_固定裁剪({ ...ctx.input }),
			ST_1大1小: async () => await ST_1大1小({ ...ctx.input }),
			横版1221: async () => await ST_横版1221({ ...ctx.input }),
			列_自适应: async () => await ST_列自适应(ctx.input),
