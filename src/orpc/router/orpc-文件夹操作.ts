...
export const ORPC_文件夹操作 = os
	...
	.handler(async (ctx) => {
		...
		const actionMap: Record<string, () => Promise<void> | void> = {
			打开素材文件夹: async () =>
				await FUN_打开文件夹(folderStructure.materialPath),
			移动到根目录: async () =>
				await FUN_移动到根目录(folderStructure.materialPath),
			移动到效果图: async () =>
				await FUN_移动到效果图(
			...
			解压ZIP: async () => await FUN_解压ZIP(folderStructure.materialPath),
			文件重命名: async () =>
				await FUN_文件重命名(folderStructure.materialPath, ctx.input.shopName),
			删除广告文件: async () =>
				await FUN_删除广告文件(folderStructure.materialPath),
			PSD导出图片: async () =>
				await FUN_PSD导出图片(folderStructure.materialPath, false),
			复制到预览图: async () =>
				await FUN_复制到预览图(
					folderStructure.materialPath,
			...
			子目录内容移动到根: async () => {
				await FUN_子目录移动到根目录(folderStructure.materialPath);
			},
			子目录重命名: async () =>
				await FUN_子目录重命名(
					folderStructure.materialPath,
			...
			享设计文件夹重构: async () =>
				await FUN_享设计文件夹重构(folderStructure.materialPath),
			子文件夹源文件重命名: async () =>
				await FUN_子文件夹源文件重命名(folderStructure.materialPath),
			打开没有预览图的AI文件: async () =>
				await FUN_打开没有预览图的AI文件(folderStructure.materialPath),
			AI导出图片: async () =>
				await FUN_AI导出图片(folderStructure.materialPath, false),
			享设计制作预览图: async () =>
				await FUN_享设计制作预览图(folderStructure.materialPath),
			图片转PSD: async () => await FUN_图片转PSD(folderStructure.materialPath),
			打开没有预览图的PSD文件: async () =>
				await FUN_打开没有预览图的PSD文件(folderStructure.materialPath),
			打开桌面上传文件夹: async () => await FUN_打开桌面上传文件夹(),
			打开小夕素材大目录: async () => await FUN_打开素材大目录("小夕素材"),
			打开饭桶设计大目录: async () => await FUN_打开素材大目录("饭桶设计"),
			打开泡泡素材大目录: async () => await FUN_打开素材大目录("泡泡素材"),
			子目录图片重命名: async () =>
				await FUN_子目录图片重命名(folderStructure.materialPath),
			"PSD导出图片—插入广告": async () =>
				await FUN_PSD导出图片(folderStructure.materialPath, true),
			"AI导出图片-插入广告": async () =>
				await FUN_AI导出图片(folderStructure.materialPath, true),
			素材图水印: async () =>
