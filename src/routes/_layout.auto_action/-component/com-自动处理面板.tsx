...
export const COM_自动处理面板 = () => {
	const getAllFolders = useMutation(orpc.orpcGetAllDirectoryList.mutationOptions());
	...
	return (
		...
		>
			...
			<form.AppField name="shopName">{(field) => <field.SelectField col={2} label="店铺" options={setting.shopList} />}</form.AppField>

			<form.AppField name="firstNum">{(field) => <field.TextField col={2} label="第一个文件夹" type="number" />}</form.AppField>

			<form.AppField name="dstPath">{(field) => <field.TextField col={8} label="目标文件夹" />}</form.AppField>
